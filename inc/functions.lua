...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(turbo..":NameBot:")
...
var = 'مطور اساسي 👨🏻‍✈️' 
elseif redis:sismember(turbo..':SUDO_BOT:',UserID) then
var = 'مطور البوت 👨🏽‍💻' 
elseif redis:sismember(turbo..':MONSHA_BOT:'..ChatID,UserID) then
var = ' المنشىء 👷🏽' 
elseif redis:sismember(turbo..'owners:'..ChatID,UserID) then
var = 'مدير البوت 👨🏼‍⚕️' 
elseif redis:sismember(turbo..'admins:'..ChatID,UserID) then
var = 'ادمن في البوت 👨🏼‍🎓' 
elseif redis:sismember(turbo..'whitelist:'..ChatID,UserID) then
var = 'عضو مميز ⭐️' 
else
var = 'فقط عضو 🙍🏼‍♂️' 
end
...
if msg.edited then
redis:incr(turbo..':edited:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.text and not msg.forward_info then
redis:incr(turbo..'msgs:'..msg.sender_user_id_..':'..msg.chat_id_)
elseif msg.adduser then 
redis:incr(turbo..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.photo then
redis:incr(turbo..':photo:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.sticker then
redis:incr(turbo..':sticker:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.voice then
redis:incr(turbo..':voice:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.audio then
redis:incr(turbo..':audio:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.video or msg.video_note then
redis:incr(turbo..':video:'..msg.chat_id_..':'..msg.sender_user_id_)
elseif msg.animation then
redis:incr(turbo..':animation:'..msg.chat_id_..':'..msg.sender_user_id_)
end
end
...
local list = redis:smembers(turbo..':SUDO_BOT:')
message = '👨🏽‍💻*¦* قائمه الـمـطـوريـن : \n\n`★`*_* '..SUDO_USER..' ➣ (' ..SUDO_ID.. '){'..redis:scard(turbo..'mtwr_count'..SUDO_ID)..'}\n*----------------------------------*\n'
if #list==0 then  message = message.."* لا يوجد مطورين حاليا \n📛 *"
else
for k,v in pairs(list) do
local info = redis:hgetall(turbo..'username:'..v)
local count = redis:scard(turbo..'mtwr_count'..v)
...
local monsha = redis:smembers(turbo..':MONSHA_BOT:'..msg.chat_id_)
if #monsha == 0 then 
...
message = message..'*----------------------------------\n\n📋¦ قائمه المدراء :*\n\n'
local list = redis:smembers(turbo..'owners:'..msg.chat_id_)
if #list == 0 then  
...
local list = redis:smembers(turbo..'admins:'..msg.chat_id_)
...
local list = redis:smembers(turbo..'whitelist:'..msg.chat_id_)
...
if redis:sismember(turbo..'is_silent_users:'..Group,User) then 
return true 
else
return false
end 
...
local list = redis:smembers(turbo..'is_silent_users:'..msg.chat_id_)
...
if redis:sismember(turbo..'banned:'..Group,User) then 
return true 
else
return false
end 
...
if redis:sismember(turbo..'gban_users',User) then 
return true 
else
return false
end 
...
local list = redis:smembers(turbo..'gban_users')
...
local list = redis:smembers(turbo..':Filter_Word:'..msg.chat_id_)
if #list ~=0 then
...
if redis:sismember(turbo..':Filter_Word:'..msg.chat_id_,word) then 
return  "📝*¦* الكلمه *{"..word.."}* هي بالتأكيد من قائمه المنع✓️" 
else
redis:sadd(turbo..':Filter_Word:'..msg.chat_id_,word) 
...
if redis:sismember(turbo..':Filter_Word:'..msg.chat_id_,word) then 
redis:srem(turbo..':Filter_Word:'..msg.chat_id_,word) 
...
EnterCmd = true
local NumberFlood = tonumber(redis:get(turbo..':Flood_Spam:'..msg.sender_user_id_..':msgs') or 0)
if NumberFlood > 3 then 
redis:setex(turbo..':Flood_Spam:'..msg.sender_user_id_..':Stoped',10,true)
EnterCmd = false
elseif redis:get(turbo..':Flood_Spam:'..msg.sender_user_id_..':Stoped') then
EnterCmd = false
else
redis:setex(turbo..':Flood_Spam:'..msg.sender_user_id_..':msgs',3,NumberFlood+1)
end
return EnterCmd
end
...
json_data = '{"BotID": '..turbo..',"UserBot": "'..Bot_User..'","Groups" : {'
local All_Groups_ID = redis:smembers(turbo..'group:ids')
for key,GroupS in pairs(All_Groups_ID) do
local NameGroup = (redis:get(turbo..'group:name'..GroupS) or '')
...
local admins = redis:smembers(turbo..'admins:'..GroupS)
if #admins ~= 0 then
json_data =  json_data..',"Admins" : {'
for key,value in pairs(admins) do
local info = redis:hgetall(turbo..'username:'..value)
if info then 
...
local creator = redis:smembers(turbo..':MONSHA_BOT:'..GroupS)
if #creator ~= 0 then
...
local owner = redis:smembers(turbo..'owners:'..GroupS)
if #owner ~= 0 then
...
local list = redis:smembers(turbo..'group:ids')
message = '📋*¦* قائمه المجموعات :\n\n'
for k,v in pairs(list) do 
local info = redis:get(turbo..'group:name'..v)
if info then 
...
redis:del(
turbo..'group:add'..id_group,
turbo..'lock_link'..id_group, 
turbo..'lock_id'..id_group,
turbo..'lock_spam'..id_group, 
turbo..'lock_webpage'..id_group,
turbo..'lock_markdown'..id_group,
turbo..'lock_flood'..id_group,
turbo..'lock_bots'..id_group,
turbo..'mute_forward'..id_group,
turbo..'mute_contact'..id_group,
turbo..'mute_location'..id_group,
turbo..'mute_document'..id_group,
turbo..'mute_keyboard'..id_group,
turbo..'mute_game'..id_group,
turbo..'mute_inline'..id_group,
turbo..'lock_username'..id_group,
turbo..'num_msg_max'..id_group,
turbo..'replay'..id_group
)
redis:srem(turbo..'group:ids',id_group)
end
...
redis:hset(turbo..'username:'..data.id_,'username',ResolveUser(data))
redis:sadd(turbo..'admins:'..msg.chat_id_,data.id_)
end,nil)
end
end
if NumAdmin == 0 then 
...
if redis:get(turbo..'group:add'..msg.chat_id_) then  return '🎗*¦* المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
redis:setex(turbo..":ismember:"..msg.sender_user_id_,1500,true)
GetFullChat(msg.chat_id_,function(arg,data) 
local GroupUsers = tonumber(redis:get(turbo..':addnumberusers') or 0)
...
local lock_service = redis:get(turbo..'lock_service')
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(turbo..'username:'..data.id_, 'username', ResolveUser(data))
redis:sadd(turbo..':MONSHA_BOT:'..msg.chat_id_,data.id_)
end)
...
redis:mset(
turbo..'group:add'..msg.chat_id_,true,
turbo..'lock_link'..msg.chat_id_,true, 
turbo..'lock_id'..msg.chat_id_,true,
turbo..'lock_spam'..msg.chat_id_,true,
turbo..'lock_webpage'..msg.chat_id_,true,
turbo..'lock_markdown'..msg.chat_id_,true,
turbo..'lock_flood'..msg.chat_id_,true,
...
turbo..'mute_keyboard'..msg.chat_id_,true,
turbo..'mute_game'..msg.chat_id_,true,
turbo..'mute_inline'..msg.chat_id_,true,
turbo..'lock_username'..msg.chat_id_,true,
turbo..'num_msg_max'..msg.chat_id_,5, 
turbo..'replay'..msg.chat_id_,true
)

redis:sadd(turbo..'group:ids',msg.chat_id_) 
redis:sadd(turbo..'mtwr_count'..msg.sender_user_id_,msg.chat_id_)

GroupTitle(msg.chat_id_,function(arg,data)
local NameGroup = data.title_
redis:set(turbo..'group:name'..msg.chat_id_,NameGroup)
...
redis:set(turbo..'linkGroup'..msg.chat_id_,Gp_Link)
...
if not redis:get(turbo..'group:add'..msg.chat_id_) then return '🛠*¦* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
...
if redis:sismember(turbo..':MONSHA_BOT:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد منشىء  \n✓️',12,USERCAR) 
end
redis:hset(turbo..'username:'..UserID,'username',Resolv)
redis:sadd(turbo..':MONSHA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تمت ترقيته ليصبح منشىء في البوت \n✓️',12,USERCAR) 
end

if cmd == "remmnsha" then
if not redis:sismember(turbo..':MONSHA_BOT:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد ليس منشىء \n✓️',12,USERCAR) 
end
redis:srem(turbo..':MONSHA_BOT:'..ChatID,UserID)
...
if redis:sismember(turbo..'whitelist:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد عضو مميز \n✓️',12,USERCAR) 
end
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..'whitelist:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n??¦ الايدي » {'..UserID..'}\n🛠¦ تمت ترقيته ليصبح عضو مميز \n✓️',12,USERCAR) 
end

if cmd == "remwhitelist" then
if not redis:sismember(turbo..'whitelist:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد ليس عضو مميز \n✓️',12,USERCAR) 
end
redis:srem(turbo..'whitelist:'..ChatID,UserID)
...
if redis:sismember(turbo..'owners:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد مدير \n✓️',12,USERCAR) 
end
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..'owners:'..ChatID,UserID)
...
if redis:sismember(turbo..'admins:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد ادمن \n✓️',12,USERCAR) 
end
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..'admins:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تمت ترقيته ليصبح ادمن \n✓️',12,USERCAR) 
end

if cmd == "remowner" then
if not redis:sismember(turbo..'owners:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد ليس مدير \n✓️',12,USERCAR) 
end
redis:srem(turbo..'owners:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم تنزيله من المدير  \n✓️',12,USERCAR) 
end

if cmd == "demote" then
if not redis:sismember(turbo..'admins:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد ليس ادمن \n✓️',12,USERCAR) 
end
redis:srem(turbo..'admins:'..ChatID,UserID)
...
if cmd == "active" then
local maseegs = redis:get(turbo..'msgs:'..UserID..':'..ChatID) or 1
...
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..'banned:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم حظره  \n✓️',12,USERCAR) 
end
end)
...
if cmd == "unban" then
if not redis:sismember(turbo..'banned:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم بالتأكيد الغاء حظره  \n✓️',12,USERCAR) 
else
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم الغاء حظره \n✓️',12,USERCAR) 
end
redis:srem(turbo..'banned:'..ChatID,UserID)
...
if redis:sismember(turbo..'is_silent_users:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم بالتأكيد كتمه  \n✓️',12,USERCAR) 
end
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..'is_silent_users:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم كتمه \n✓️',12,USERCAR) 
end

if cmd == "unsilent" then
if not redis:sismember(turbo..'is_silent_users:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم بالتاكيد الغاء كتمه  \n✓️',12,USERCAR) 
end
redis:srem(turbo..'is_silent_users:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم الغاء كتمه  \n✓️',12,USERCAR) 
end
...
redis:hset(turbo..'username:'..UserID, 'username',Resolv)
redis:sadd(turbo..'gban_users',UserID)
...
redis:srem(turbo..'gban_users',UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم الغاء حظره العام \n✓️',12,USERCAR) 
end
...
if redis:sismember(turbo..':SUDO_BOT:',UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد مطور \n✓️',12,USERCAR) 
end
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..':SUDO_BOT:',UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم ترقيته ليصبح مطور \n✓️',12,USERCAR) 
end

if cmd == "dn_sudo" then
if not redis:sismember(turbo..':SUDO_BOT:',UserID) then 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ انه بالتأكيد ليس مطور \n✓️',12,USERCAR) 
end
redis:srem(turbo..':SUDO_BOT:',UserID)
...
redis:hset(turbo..'username:'..UserID, 'username', UserName)
redis:sadd(turbo..':MONSHA_BOT:'..ChatID,UserID)
return sendMsg(ChatID,MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تمت ترقيته ليصبح منشىء في البوت \n✓️')
end
...
if redis:sismember(turbo..'banned:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'👤*¦* العضو » '..UserName..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* تم بالتأكيد حظره \n✓️') 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
Restrict(ChatID,UserID,1)
redis:hset(turbo..'username:'..UserID, 'username', Resolv)
redis:sadd(turbo..':tqeed:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم تقييد آلعضـو بنجآح \n✓️',12,USERCAR) 
end 
if cmd =="fktqeed" then
Restrict(ChatID,UserID,2)
redis:srem(turbo..':tqeed:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم فگ تقييد آلعضـو بنجآح \n✓️',12,USERCAR) 
end
if cmd == "setwhitelist" then
if redis:sismember(turbo..'whitelist:'..ChatID,UserID) then 
...
if cmd == "dn_sudo" then
if not redis:sismember(turbo..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'👤*¦* العضو » '..USERNAME..' \n🎫*¦* الايدي » (`'..UserID..'`)\n🛠*¦* انه بالتأكيد ليس مطور \n✓') end
redis:srem(turbo..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'👤¦ العضو » '..USERNAME..' \n🎫¦ الايدي » {'..UserID..'}\n🛠¦ تم تنزيله من المطورين \n✓️',12,USERCAR) 
end
...
if redis:get(turbo..'welcome:get'..msg.chat_id_) then welcomex = "{ مفعل }" else welcomex = "{ معطل }" end
if redis:get(turbo..'lock_id'..msg.chat_id_) then lock_idx = "{ مفعل }" else lock_idx = "{ معطل }" end
if redis:get(turbo..'replay'..msg.chat_id_)  then replayx  = "{ مفعل }" else replayx  = "{ معطل }" end
if redis:get(turbo..'lock_woring'..msg.chat_id_) then lock_woringx = "{ مفعل }" else lock_woringx  = "{ معطل }" end
list_settings = "*👮🏾¦*` اعدادات المجموعه :` \n"
.."\n📝¦ التعديل » "..(redis:get(turbo..'lock_edit'..msg.chat_id_) or 'false')
.."\n🔗¦ الروابط » "..(redis:get(turbo..'lock_link'..msg.chat_id_) or 'false')
.."\n#️⃣¦ التاك » "..(redis:get(turbo..'lock_tag'..msg.chat_id_) or 'false')
.."\n©¦ المعرفات » "..(redis:get(turbo..'lock_username'..msg.chat_id_) or 'false')
.."\n\n💱¦ التكرار » "..(redis:get(turbo..'lock_flood'..msg.chat_id_) or 'false')
.."\n📑¦ الكلايش » "..(redis:get(turbo..'lock_spam'..msg.chat_id_) or 'false')
.."\n🌐¦ الويب » "..(redis:get(turbo..'lock_webpage'..msg.chat_id_) or 'false')
.."\n⚜️¦ الماركدوان » "..(redis:get(turbo..'lock_markdown'..msg.chat_id_) or 'false')
.."\n🏌🏻¦ البوتات بالطرد » "..(redis:get(turbo..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n🤖¦ البوتات » "..(redis:get(turbo..'lock_bots'..msg.chat_id_) or 'false')
.."\n➕¦ عدد التكرار » "..(redis:get(turbo..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n🎬¦` اعدادات الوسائط :`\n"
.."\n*¦* المتحركه » "..(redis:get(turbo..'mute_gif'..msg.chat_id_) or 'false')
.."\n💭¦ الدردشه » "..(redis:get(turbo..'mute_text'..msg.chat_id_) or 'false')
.."\n◽️¦ الانلاين » "..(redis:get(turbo..'mute_inline'..msg.chat_id_) or 'false')
.."\n🎮¦ الالعاب » "..(redis:get(turbo..'mute_game'..msg.chat_id_) or 'false')
.."\n🏞¦ الصور » "..(redis:get(turbo..'mute_photo'..msg.chat_id_) or 'false')
.."\n🎥¦ الفيديو » "..(redis:get(turbo..'mute_video'..msg.chat_id_) or 'false')
.."\n🎙¦ البصمات » "..(redis:get(turbo..'mute_audio'..msg.chat_id_) or 'false')
.."\n\n🔉¦ الصوت » "..(redis:get(turbo..'mute_voice'..msg.chat_id_) or 'false')
.."\n🎎¦ الملصقات » "..(redis:get(turbo..'mute_sticker'..msg.chat_id_) or 'false')
.."\n📞¦ الجهات » "..(redis:get(turbo..'mute_contact'..msg.chat_id_) or 'false')
.."\n💱¦ التوجيه » "..(redis:get(turbo..'mute_forward'..msg.chat_id_) or 'false')
.."\n🌐¦ الموقع » "..(redis:get(turbo..'mute_location'..msg.chat_id_) or 'false')
.."\n🗂¦ الملفات » "..(redis:get(turbo..'mute_document'..msg.chat_id_) or 'false')
.."\n🔅¦ الاشعارات » "..(redis:get(turbo..'mute_tgservice'..msg.chat_id_) or 'false')
.."\n🔒¦ الكيبورد » "..(redis:get(turbo..'mute_keyboard'..msg.chat_id_) or 'false')
.."\n\n*⚒¦*` اعدادات اخرى : `"
.."\n*🙋🏼‍♂️¦* الترحيب » "..welcomex
.."\n*💬¦*  الردود » "..replayx
...
.."\n*➕¦* عدد التكرار » "..(redis:get(turbo..'num_msg_max'..msg.chat_id_) or 'false')
.."\n\n*💱¦*` اعدادات التقـييد :`\n"
.."\n*💢¦* التقييد بالتوجيه » "..(redis:get(turbo..':tqeed_fwd:'..msg.chat_id_) or 'false')
.."\n*📸¦* التقييد بالصور » "..(redis:get(turbo..':tqeed_photo:'..msg.chat_id_) or 'false')
.."\n*🔗¦* التقييد بالروابط » "..(redis:get(turbo..':tqeed_link:'..msg.chat_id_) or 'false')
.."\n*🎉¦* التقييد بالمتحركه » "..(redis:get(turbo..':tqeed_gif:'..msg.chat_id_) or 'false')
.."\n*🎥¦* التقييد الفيديو » "..(redis:get(turbo..':tqeed_video:'..msg.chat_id_) or 'false')
list_settings = list_settings:gsub('true', '{ مقفول }')
list_settings = list_settings:gsub('false', '{ مفتوح }')
